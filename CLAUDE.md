...
```
~/.cdp-bridge/
├─ chrome-profile/           # sidecar Chrome 用的 user-data-dir（仅 spawn 模式创建）
│  └─ Default/...             # 从用户主 profile 选择性拷贝
├─ downloads/                 # cdpb fetch staging — fetch 结束自动清理（不留孤儿）
├─ state.json                 # { mode, pid?, port, proxy?, profileSyncedAt?, lastStoppedAt? }
├─ .lock                      # 原子 O_EXCL 文件锁，stateful 命令运行期间持有
...
- `cdpb stop`：spawn 模式两段式（先 CDP `Browser.close` 让 Chrome 刷盘，5s 等不到自然退出再 `taskkill /T /F` 兜底；日志标 `(graceful close)` 或 `(force kill)`）；attach 模式只清 state.json，**永远不杀你日常 Chrome**
- `cdpb setup-shortcut [--dry-run] [--revert] [--include-registry] [--force]`：改你 Chrome 快捷方式加 CDP 启动参数。检测到 chrome.exe 还在跑时默认拒绝（防止 Chrome 单实例 pickup 把新 flag 吞掉），`--force` 跳过这个检测
...
- `cdpb screenshot <url> [-o <path>] [--full-page]`：用 Chrome 打开页面截 PNG，background tab 不抢焦点
- `cdpb exec <url> <js>`：在页面中执行 JS 并取返回值（替代 web-access /eval）
- `cdpb tab list|new|close`：tab 管理（list 列出所有 page 类型 target，new 创建新 tab，close 关闭）
